...
def from_matrix_to_transform(matrix, stamp, frame_id, child_frame_id,
    ...
    t.transform.rotation.w = sqrt( max(0, 1 + matrix[0][0] + matrix[1][1] + matrix[2][2]) ) / 2.0
    t.transform.rotation.x = sqrt(max(0, 1 + matrix[0][0] - matrix[1][1] - matrix[2][2])) / 2.0
    t.transform.rotation.y = sqrt(max(0, 1 - matrix[0][0] + matrix[1][1] - matrix[2][2])) / 2.0
    t.transform.rotation.z = sqrt(max(0, 1 - matrix[0][0] - matrix[1][1] + matrix[2][2])) / 2.0
    t.transform.rotation.x = copysign(t.transform.rotation.x, matrix[2][1] - matrix[1][2])
    t.transform.rotation.y = copysign(t.transform.rotation.y, matrix[0][2] - matrix[2][0])
    t.transform.rotation.z = copysign(t.transform.rotation.z, matrix[1][0] - matrix[0][1])
...
if __name__ == '__main__':
    ...
    # joy_left_pub = rospy.Publisher('vive_left_controller', Joy, queue_size=1)
    # prev_unPacketNum_left = 0
    # joy_right_pub = rospy.Publisher('vive_right_controller', Joy, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        # Left controller pose tracking
        if left_id:
            matrix = poses[left_id].mDeviceToAbsoluteTracking
            print("LEFT CONTROLLER position x: ", matrix[0][3])
            print("LEFT CONTROLLER position y: ", matrix[1][3])
            print("LEFT CONTROLLER position z: ", matrix[2][3])
        ...
        # Right controller pose tracking
        if right_id:
